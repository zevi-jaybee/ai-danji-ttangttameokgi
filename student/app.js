...
const lessons = [
  {
    ...
    actions: ['Codex를 실행하고 본인 계정으로 로그인합니다. 계정에서 로컬 작업을 사용할 수 있는지 확인합니다.', '실습 폴더를 선택해 로컬 작업을 엽니다. 웹 브라우저의 일반 채팅에만 입력하면 이 PC의 설치가 진행되지 않습니다.', '아래 내용을 보낸 뒤 필요한 승인·QGIS 실행·재시작 안내를 따릅니다.'],
...
마지막에 설치·변경한 항목, 실제 버전, 검사 결과, 아직 내가 할 일을 짧게 정리해. 설계 데이터는 편집하지 마.`,
  ...
  },
  {
    ...
    lead: 'QGIS가 실제로 연결되는지 확인합니다. 문제가 있으면 Codex에게 원인을 찾고 연결 설정을 고치도록 요청합니다.',
    actions: ['QGIS를 실행합니다. 여러 창이 열려 있다면 작업할 창을 확인합니다.', '수업 배포 QGZ를 엽니다. QGZ와 GPKG 등 원본 자료의 상대 폴더 구조를 유지합니다.', 'Codex에서 아래 요청문을 실행합니다. 자료가 없다면 프로젝트 읽기 확인은 나중에 합니다.'],
    prompt: `QGIS MCP 연결을 점검하고, 문제가 있으면 원인을 찾아 연결 설정을 고쳐 줘.
